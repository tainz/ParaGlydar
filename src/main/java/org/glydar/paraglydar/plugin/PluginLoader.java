...
public class PluginLoader {
	...
	public void loadPlugins() {
		...
		ParaGlydar.getServer().getLogger().info("Loading plugins...");
		for (File file : pluginDir.listFiles()) {
			if (!file.isDirectory() && !file.getName().startsWith(".")) {
				try {
					loadPlugin(file);
				} catch (PluginException e) {
					ParaGlydar.getServer().getLogger().warning(e.getMessage());
				}
			}
		}
		if (ParaGlydar.getServer().isRunning()) {
			for (Plugin p : pending) {
				try {
					enablePlugin(p);
				} catch (Exception e) {
					ParaGlydar.getServer().getLogger().warning("Error initializing plugin " + p.getName());
					e.printStackTrace();
				}
			}
		}
		ParaGlydar.getServer().getLogger().info("Loaded " + loadedPlugins.size() + " plugins!");
	}

	public void unloadPlugins() {
		for (Plugin plugin : loadedPlugins) {
			ParaGlydar.getServer().getLogger().info("Disabling " + plugin.getName() + " v" + plugin.getVersion());
	...
	public void loadPlugin(File file) throws PluginException {
		...
		try {
			...
			ParaGlydar.getServer().getLogger().warning("Failed to load file " + file.getName() + "! " + e.getMessage());
			e.printStackTrace();
			return;
		}
		...
		try {
			...
			ParaGlydar.getServer().getLogger().warning("failed to load plugin " + plugin.getName() + "! " + e.getMessage());
			e.printStackTrace();
			return;
		}
		...
		ParaGlydar.getServer().getLogger().info(sb.toString());
		pending.add(plugin);
	}
	...
	private Plugin getPlugin(File file) throws PluginException, NoSuchMethodException, SecurityException, IOException,
		...
		try {
			...
			try {
				...
				if (c.getSuperclass() == Plugin.class) {
					...
					while (e.hasMoreElements()) {
						...
						try {
							...
							ParaGlydar.getServer().getLogger().warning("Error loading plugin class from " + className);
						}
					}
				} else {
				...
				ParaGlydar.getServer().getLogger().warning(ex.getMessage());
			}
		} catch (Exception ex) {
			while (e.hasMoreElements()) {
				JarEntry entry = e.nextElement();
